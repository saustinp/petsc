/*
   Phase 4b validator — PC_LEFT + PCJACOBI on cdr_small.
...
int main(int argc, char **argv)
{
  ...
  PetscCall(PetscInitialize(&argc, &argv, NULL, "Phase 4b — PC_LEFT + PCJACOBI gate"));
  ...
  if (rank == 0) printf("[pcleft-jacobi] loaded cdr_small: N=%" PetscInt_FMT " on %d ranks\n", N, (int)size);
  ...
  if (rank == 0) {
    ...
    /* Self-consistency: KSP-reported rnorm and externally-measured residual
       ...
       PC_LEFT unwrap is missing or wrong. With PCJACOBI on cdr_small the
